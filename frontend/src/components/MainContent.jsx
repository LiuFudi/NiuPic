...
function MainContent() {
  ...
  const loadImages = useCallback(async (isInitialLoad = true) => {
    ...
    // 未选文件夹 = 看全部图片（不传 folder，后端返回所有层级的图片）
    // 未搜索、无筛选同理：照常加载，不再提前 return
    // 暂停空闲加载并取消之前的所有请求（关键！）
    onUserActionStart();
  ...
  // 恢复撤销前的文件夹状态
  useEffect(() => {
    const restoreFolder = localStorage.getItem('niupic_restore_folder');
    if (restoreFolder) {
      const { setSelectedFolder } = useImageStore.getState();
      setSelectedFolder(restoreFolder);
      localStorage.removeItem('niupic_restore_folder');
