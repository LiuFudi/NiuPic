# NiuPic 飞牛 NAS 打包部署指南

本指南介绍如何将 NiuPic 打包成飞牛 fnOS 应用包（.fpk）并部署到飞牛 NAS。
...
构建过程：
1. 构建前端 (Vite)
2. 复制后端代码到 `niupic/app/server/`
3. 复制前端产物到 `niupic/app/server/public/`
4. 使用 Docker 安装 Linux glibc 版本依赖
5. 修复脚本换行符 (CRLF → LF)

构建完成后，`niupic` 目录即可上传到飞牛。

## 二、上传到飞牛 NAS

将整个 `niupic` 文件夹上传到飞牛 NAS，可以使用：
- 飞牛文件管理器
- SFTP/SCP
- SMB 共享

建议上传到用户目录，如：`/vol1/1000/niupic`
...
成功后会生成 `niupic_1.0.0_all.fpk` 文件。
...
### 方法二：命令行安装
```bash
appcenter-cli install-fpk niupic_1.0.0_all.fpk
```
...
1. **配置授权目录**：在飞牛应用设置中，为 NiuPic 添加素材库文件夹的访问权限

2. **添加素材库**：打开 NiuPic，添加素材库时输入完整路径，如：
   - `/vol1/1000/图片库`
   - `/vol2/photos`
...
appname=niupic
version=1.0.0
desc=牛图 - 专为飞牛 fnOS 设计的轻量、快速、稳定的图像素材检索浏览应用
arch=x86_64
display_name=NiuPic
...
2. 检查应用日志：`/var/apps/niupic/var/info.log`
3. 确保端口 15002 未被占用
...
A: SSH 登录后查看：
```bash
cat /var/apps/niupic/var/info.log
```
