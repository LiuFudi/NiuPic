...
class LibraryDatabase {
  constructor(libraryPath) {
    ...
    this.niupicDir = path.join(libraryPath, '.niupic');
    this.dbPath = path.join(this.niupicDir, 'metadata.db');
    
    // Ensure .niupic directory exists
    if (!fs.existsSync(this.niupicDir)) {
      fs.mkdirSync(this.niupicDir, { recursive: true });
    }
    
    // Create thumbnails directory (使用分片结构: .niupic/thumbnails/ab/hash.webp)
    const thumbDir = path.join(this.niupicDir, 'thumbnails');
  ...
  /**
   * 只更新原图分辨率（宽 x 高）
   * 用于纠正历史数据里被误写成缩略图尺寸的 width/height。
   */
  updateImageDimensions(id, width, height) {
    const stmt = this.db.prepare('UPDATE images SET width = ?, height = ? WHERE id = ?');
    const result = stmt.run(width, height, id);
    this.updateLastModified();
    return result;
  }
